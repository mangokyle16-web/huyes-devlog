...
**2026-06-12 補充（訓練完成）：**
- YOLOX-tiny 50 epochs 完成：**Best AP@50:95 = 95.01%**
- `yolox_tiny_beans_final.hef`（9.76MB）部署到 Pi5
- ONNX patch pipeline 標準化：extract_heads → 移除opset18屬性 → 補Conv kernel_shape → DFC 編譯

**2026-06-12 補充（Pi5 部署完成 + Bug 修正）：**
- YOLOX-tiny 整合進 Pi5 採集 UI，即時偵測運作正常
- Pi5 INT8 批次測試發現偵測數偏低，修正 2 個 detector bug：
  1. 預處理改 letterbox（原 cv2.resize 拉伸失真）
  2. 移除 MAX_BRIGHTNESS 亮度過濾（誤殺過曝亮豆，frame_805 從 0→8）
- 修正後 8 張 Pi5 INT8 = Mac FP32，YOLOX INT8 幾乎無損確認
- 完整開發紀錄 + 修正前後對比圖：`docs/yolox_deployment/`

---

## 2026-06-13

**豆子計數系統 — 重複計算問題解決 + 全域運動補償追蹤**

**桌面化 + 計數 UI(Codex 實作):**
- Pi5 桌面圖示「Huyes 採集」一鍵啟動(launch_ui.sh + .desktop)
- UI 右上角即時累積豆數 + 暫停採集按鈕→存批次 JSON(每frame豆數+座標+總數)

**核心問題:輸送帶豆子重複計算**
- 原本每幀把當前豆數加總 → 同一顆豆停留多幀被重複計
- 改 SORT-lite 追蹤後反而嚴重漏計(40顆只數到2)

**根因診斷(用真實 120s 採集數據分析):**
- 偵測只有 2-3 fps(qsToGray 多光譜轉換 ~300ms 是瓶頸)
- 相機產 669 幀,偵測只跑 318 幀 → 掉幀 53%
- 豆子每步移動 140-160px = 一個框寬 → 同一顆豆前後幀近乎零 IOU
- 逐顆 IOU/中心追蹤數學上不可行
- 互相關測得全域流速:+160px/幀,91% 往右(豆子確實左→右直線流動)

**解法:全域運動補償追蹤(Codex 實作,離線驗證後部署)**
- 每幀以 x 重心直方圖互相關估全域流速
- 軌跡用全域流速往前推 → 跟預測位置配對(非舊位置)
- 觀測中心跨越計數線(x=800)時計一次
- 離線驗證:真值 399 → 算出 368(92.2%);實機 399 → 330(83%)

**精度差距根因:掉幀**
- 偵測掉 53% 幀使每步移動加倍至一個框寬,追蹤窗口易錯過
- qsToGray 改 grayscale 只從 2.0→3.1fps(光譜 demosaic 本身慢)

**進行中:方案 A — raw mosaic 快速灰階**
- 跳過廠商 qsToGray,直接 block-average 感測器 mosaic → 灰階(NEON+4核)
- 目標 10+fps,豆子每步移動降到 ~35-40px,追蹤大幅改善
- 代價:YOLOX 需在新影像格式重訓(標註座標可沿用)
- Codex 分析 SDK raw 格式中

**模型部署紀錄(YOLOX-tiny 取代 YOLOv8n):**
- 詳見 docs/yolox_deployment/(YOLOv8n DFL INT8 失敗 → YOLOX 直接回歸無損)

**2026-06-13 補充(方案A 完成,計數準確率提升):**
- fast-gray 部署實機:偵測 fps 2.5→5.4,豆子每步移動 140-160px→70-80px
- 計數準確率:真值 399 → 370(93%),較先前 83% 大幅提升
- 瓶頸轉移到相機出幀率(~5.6fps,曝光允許更高,SDK配置限制)
- docs/planA_fastgray/ 完整紀錄 + 同幀比較圖
